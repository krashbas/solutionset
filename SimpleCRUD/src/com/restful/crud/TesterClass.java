...
public class TesterClass {
  public static void main(String[] args) {
    ...
    // create one todo
    Todo todo = new Todo("3", "Blabla", LocalDateTime.now().plusDays(1).toString());
...
//    System.out.println(service.path("rest").path("todos").request().accept(MediaType.APPLICATION_JSON).get(String.class));
    ...
    form.param("complete-data", LocalDateTime.now().plusDays(3).toString());
  ...
  private static URI getBaseURI() {
    return UriBuilder.fromUri("http://localhost:8080/SimpleCRUD").build();
  }
} 
